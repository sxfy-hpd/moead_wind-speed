...
def main():
    ...
    seq_len = 9
    vmd_k = 5
    x_trains, y_trains, x_valids, y_valids, x_tests, y_tests, u = load_data_with_vmd(wind_speed, vmd_k, seq_len)
    predict_valids = []
    models = []
    ...
    MOEAD.save_filename = "result/differentK/" + filename + step
    ...
    save_opt_pop.to_csv('result/differentK/' + filename + step + '/pareto.csv', header=False, index=False)
    ...
    for idx, pre in enumerate(result):
        ...
        up.append(pre + lambdas + 0.45)
        down.append(pre - lambdas - 0.45)
    ...
    final_result.to_csv('result/differentK/' + filename + step + f'/picp:{test_picp:.5f}-pinrw:{test_pinrw:.5f}-cwc:{test_cwc:.5f}.csv', header=False, index=False)
