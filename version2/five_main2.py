...
def main():
    filename = "washingtong_winter_20120101-20120107新"
    step = "/svr/three"
    ...
    # 验证集加入一起训练
    new_models = []
    new_x_trains = np.hstack((x_trains,x_valids))
    new_y_trains = np.hstack((y_trains, y_valids))
    for i in range(0,len(models)):
        if models[i][1] == 1:
            new_model = gru(seq_len, 0.01, 32)
            callback = tf.keras.callbacks.EarlyStopping(monitor='loss', patience=10, restore_best_weights=True)
            new_model.fit(new_x_trains[i], new_y_trains[i], epochs=150, batch_size=64, callbacks=[callback], verbose=1)
            new_models.append([new_model, 1])
        elif models[i][1] == 2:
            new_model = svr()
            x = np.squeeze(new_x_trains[i])
            y = np.squeeze(new_y_trains[i])
            new_model.fit(x, y)
            new_models.append([new_model, 2])
        elif models[i][1] == 3:
            sequence = []
            for idx in range(len(new_x_trains[i])):
                if idx == 0:
                    for s in range(seq_len):
                        sequence.append(new_x_trains[i][idx][s][0])
                sequence.append(new_y_trains[i][idx][0])
            new_models.append([models[i][0], 3])
        ...
        if new_models[i][1] == 1:
            ...
            for index in range(0, len(x_tests[i])-ppp, 3):
                ...
                for num in range(3):
                    ...
                    x = x.reshape(1, -1)
                    predict_test.append(forecast)
        elif new_models[i][1] == 3:
            predict_test = []
            for index in range(0, len(x_tests[i])-ppp, 3):
                print(index, len(x_tests[i]))
                for num in range(3):
                    new_models[i][0].fit(sequence)
                    forecast = new_models[i][0].predict(n_periods=1)
                    sequence = sequence[1:]
                    sequence = np.append(sequence, forecast)
                    predict_test.append(forecast)
                sequence = sequence[:-3]
                sequence = np.append(sequence, y_tests[i][index][0])
                sequence = np.append(sequence, y_tests[i][index+1][0])
                sequence = np.append(sequence, y_tests[i][index+2][0])
    ...
    for idx, pre in enumerate(result):
        ...
        up.append(pre + lambdas + 0.6)
        down.append(pre - lambdas - 0.6)
